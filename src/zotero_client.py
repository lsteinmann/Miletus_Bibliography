...
class ZoteroClient:
    ...
    def _get_total_items(self) -> int:
        ...
        response = requests.get(
            self.base_url, headers=self.headers, params={"limit": 1, "v": 3}
        )
        response.raise_for_status()
        return int(response.headers.get("Total-Results", 0))

    def _fetch_items_batch(
        self, start_index: int, format_type: str = "csv", batch_size: int = 100
    ) -> str:
        """
        ...
        """
        params = {"limit": batch_size, "start": start_index, "v": 3}
    ...
    def _fetch_items(self, format_type: str = "csv", limit: int = 0) -> List[str]:
        ...
        for i, start_index in enumerate(sequences):
            print(
                f"Fetching batch {i+1}/{len(sequences)} (items {start_index}-{min(start_index+batch_size, fetch_limit)})"
            )

            try:
                response_data = self._fetch_items_batch(
                    start_index, format_type, batch_size
                )
