...
from src.language_services import transliterate, get_sorting_alphabet
from src.language_services import sort_turkish, turkish_sort_key
...
class BibliographyClient:
    def __init__(self, json: List[Dict[str, Any]] = None, tags: "TagClient" = None):
        ...
        if not json:
            raise Exception("BibliographyClient needs the json-Exports")
        self.tags = tags
    ...
    def __process_json(self, json: List[Dict[str, Any]] = None):
        ...
        print(
            " -- Warnings may appear here; see the output of the DataChecker for more info --"
        )
        print("---")
        for item in json:
            data = item.get("data", {})
            ...
            if citationKey:
                ...
                # citationKey. Otherwise the whole TeX-concept
                # will not work at all.
                if key not in self.keys_to_data:
                    self.keys_to_data[key] = data
                else:
                    print(
                        f"WARNING: Item {key} alreaedy exists. This should not have happened."
                    )
            ...
            for creator in creators:
                ...
                if role == "editor":
                    ...
                    # don't know what is going on and want to err on the side
                    # of inclusion.
                    continue
                # I checked what using transliteration as key did to the length
                # of the author list and it went from 1085 (without) to 1084
                # (with). Now, I do not know if this is good or bad,
                # but I assume it is good. If I wanted to be very diligent
                # I should check who got merged, but I won't do that now.
                ...
                # If an author is listed twice, e.g. as bookAuthor
                # and also author, they would be added twice and the
                # key would be added twice to their entry. Therefore in
                # both cases I check if the author_key exists already,
                # and down there if the key already exists for the author.
    ...
    def __prepare_letter_groups(self):
        ...
        for author in sorted_authors:
            ...
            if first_letter in authors_by_letter:
                ...
                print(
                    f"WARNING! -- Did not anticipate {first_letter} showing up, and it is thus not in the sorting alphabet."
                )
    ...
    def _sort_by_author_and_year(
        ...
        def sort_key(item: Dict[str, str]) -> Tuple[Tuple, int, int]:
            ...
            if year_raw is None:
                year_int = -1
                # or float('-inf') if you want None to come first
            else:
                try:
    ...
    def get_data(self, key) -> Optional[Dict[str, None]]:
        ...
        Returns:
            Dict: containing e.g. DOI, date, dateAdded, dateModified,
            creators (List[Dict]). citationKey, title, and much more.
            All the entered data from Zotero.
        """
    ...
    def get_citationKey(self, key) -> Optional[str]:
        ...
        Returns:
            str: citationKey as referenced in the .bib-files used
                for building LaTeX citations.
        """
        if key in self.keys_to_data:
    ...
    def get_author(self, author: Tuple[str, str]) -> Dict[str, None]:
        ...
        Args:
            author (Tuple(str, str)): The author-key
                (Tuple of (lastName, firstName))

        Returns:
            Dict with list of item-keys, first and last name in
                original and transliterated state.
        """
    ...
    def get_sorted_tag_groups(self) -> Dict[str, List[str]]:
        ...
        for tag in all_tags:
            if tag == "01 Grabungs und Arbeitsberichte":
                ...
                # everything sorted by year only - we don't care
                # who authored it. This is the timeline.
                ...
                continue
            # And this is for *all* other sections,
            ...
            for child_tag in child_tags:
                ...
                # This simply won't run if the tag has no children.
                for grandchild_tag in grandchild_tags:
                    keys_in_subsections.extend(self.get_keys_by_tag(grandchild_tag))
            # Here we remove all items/keys that are present in
            # one of the sub-sections of this section from the
            # current set of keys. Why are we doing this? All items are
            # tagged with multiple tags. An item that would be tagged with
            # one of the "sub" or "sub-sub"-section tags will also be
            # tagged with the highest section, usually.
            # But we don't need all items to appear in these "broader" sections
            # when it will be clear from the structure of the pdf that
            # they belong to that larger group.
...
if __name__ == "__main__":
    ...
    if demo:
        ...
        # Get a Dict of all authors indexed by the
        # Tuple of (lastName, firstName)
        ...
        # Get a Lis of all keys belongig to the
        # author-key (lastName, firstName)
        print(bib.get_author(("Steinmann", "Lisa")))
        # Get a Dict of all keys and their respective
        # author-key (lastName, firstName)
        ...
        # Sorting all authors... is annoying, since we do want to use
        # the turkish locale for sorting in this Bibliography.
        # Since setting the locale isn't super reliable especially
        # in runners, we just use our own turkish-sort:
